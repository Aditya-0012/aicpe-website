...
export default function LifeMemberForm() {
  ...
  const onSubmit = async (data) => {
    setLoading(true);
    try {
      const res = await submitMember('life', data);
      setSuccess(res.data.membershipId);
      toast.success('Life membership application submitted!');
    } catch (err) {
  ...
  if (success) return (
    <div className="form-page page-enter">
      <div className="form-header form-header--gold">
        <div className="container"><h1 className="form-header-title">Life Member</h1></div>
      </div>
      <div className="container form-body">
        <div className="form-card">
          <SuccessScreen membershipId={success} memberType="Life" onReset={() => { reset(); setSuccess(null); }} />
        </div>
      </div>
    </div>
  );
  ...
  return (
    <FormWrapper
      title="Life Member Registration"
      subtitle="Lifetime membership for dedicated physical education professionals"
      fee="₹5,000 one-time"
      icon="⭐"
      color="gold"
    >
      <form onSubmit={handleSubmit(onSubmit)} noValidate>
        ...
        <FormSection title="Contact Information">
          ...
          <FormRow cols={1}>
            <div className="form-group">
              <label className="form-label">Postal Address <span>*</span></label>
              <textarea className={`form-textarea ${errors.address ? 'error' : ''}`}
                rows={2} placeholder="House/Flat No, Street, Area" style={{ minHeight: 72 }}
        ...
        <FormSection title="Academic & Professional Details">
          <FormRow cols={2}>
            <div className="form-group">
              <label className="form-label">Highest Qualification <span>*</span></label>
              <select className={`form-select ${errors.highestQualification ? 'error' : ''}`}
              ...
              {errors.highestQualification && <span className="form-error">{errors.highestQualification.message}</span>}
            </div>
            <div className="form-group">
              <label className="form-label">Specialization</label>
              <input className="form-input" placeholder="e.g. Sports Psychology, Biomechanics"
                {...register('specialization')} />
            </div>
          </FormRow>
          <FormRow cols={2}>
            <div className="form-group">
              <label className="form-label">Current Designation <span>*</span></label>
              <input className={`form-input ${errors.designation ? 'error' : ''}`}
                placeholder="e.g. Professor, Director of Sports"
                {...register('designation', { required: 'Designation is required' })} />
              {errors.designation && <span className="form-error">{errors.designation.message}</span>}
            </div>
            <div className="form-group">
              <label className="form-label">Institution / Organization <span>*</span></label>
              <input className={`form-input ${errors.institution ? 'error' : ''}`}
          ...
          <FormRow cols={2}>
            <div className="form-group">
              <label className="form-label">Total Experience <span>*</span></label>
              <select className={`form-select ${errors.totalExperience ? 'error' : ''}`}
                {...register('totalExperience', { required: 'Experience is required' })}>
            ...
            <div className="form-group">
              <label className="form-label">No. of Research Papers Published</label>
              <input className="form-input" placeholder="e.g. 12"
        ...
        <FormSection title="Payment Information">
          ...
          <div style={{ background: 'var(--saffron-light)', border: '1px solid #f5c279', borderRadius: 8, padding: '12px 16px', fontSize: '0.85rem', color: '#92400e', marginBottom: 20 }}>
            <strong>Life Membership Fee: ₹5,000 (one-time)</strong> | DD/Cheque in favour of "AICPE" payable at Bhopal. Online: UPI cp@aicpe
          </div>
        </FormSection>

        <div className="form-submit-area">
          <p className="form-submit-note">By submitting, you agree to AICPE's lifetime membership terms and conditions.</p>
          <button type="submit" disabled={loading} className="btn btn-saffron btn-submit">
